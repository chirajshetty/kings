...
import com.example.demo.sudoku.data.ISudokuBoard;
import com.example.demo.sudoku.data.SudokuBoard;
...
public class SudokuSolver implements ISudokuSolver {
	...
	public static void main(String[] args) {
		...
		int[][] board = { { 8, 0, 0, 7, 5, 3, 0, 0, 0 }, { 0, 0, 3, 6, 0, 0, 0, 0, 0 }, { 0, 7, 0, 0, 9, 0, 2, 0, 0 },
	...
	private static boolean solve(ISudokuBoard sudokuBoard) {
			...
			for (int j = 0; j < 9; j++) {
				if (sudokuBoard.getValue(i, j) == 0) {
					for (int k = 1; k < 10; k++) {
						...
						if (isValidRow(sudokuBoard.getArray(), i, j, k)
								&& isValidColoumn(sudokuBoard.getArray(), i, j, k)
								&& isValidBox(sudokuBoard.getArray(), i, j, k) && solve(sudokuBoard)) {
							return true;
						}
	...
	public String sudoku() {
		...
		executionStart();
		ISudokuBoard sudokuBoard = new SudokuBoard(board);
	...
	@Override
	public String sudoku(int[][] board) {
